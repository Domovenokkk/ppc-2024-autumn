...
TEST(mezhuev_m_most_different_neighbor_elements, PostProcessingValidOutputTest) {
  boost::mpi::environment env;
  boost::mpi::communicator world;
  ...
  taskData->outputs.resize(2);
  taskData->outputs[0] = reinterpret_cast<uint8_t*>(new int[1]{1});
  taskData->outputs[1] = reinterpret_cast<uint8_t*>(new int[1]{0});
  taskData->outputs_count.resize(1);
  taskData->outputs_count[0] = 1;
